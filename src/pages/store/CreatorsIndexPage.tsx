  ...
  return (
    <div className="container py-10">
      ...
      {isLoading ? (
        ...
        <QueryError message="Couldn't load creators." onRetry={() => refetch()} />
      ) : (
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
          {(data ?? []).map((c: any) => (
            <Link key={c.id} to={`/creator/${c.id}`} className="bg-card border border-border rounded-lg p-5 hover:border-primary transition-colors">
              <div className="flex items-center gap-3 mb-3">
                <div className="w-12 h-12 rounded-full bg-surface overflow-hidden">{c.logo_url && <img src={c.logo_url} alt={`${c.brand_name} logo`} loading="lazy" className="w-full h-full object-cover" />}</div>
                <div>
