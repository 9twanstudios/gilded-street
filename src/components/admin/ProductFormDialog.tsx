          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
                ...
                <div className="space-y-3">
                  ...
                  <div className="flex items-center gap-2">
                    <Button type="button" variant="outline" size="sm" onClick={() => fileInputRef.current?.click()}
                      disabled={uploading} className="border-border text-muted-foreground text-xs">
                      <ImageIcon className="h-3 w-3 mr-1" />
                      {uploading ? "Uploading..." : "Upload Image"}
                    </Button>
                    <span className="text-xs text-muted-foreground">or</span>
                    <Input placeholder="Paste image URL" value={field.value}
                      onChange={(e) => { field.onChange(e.target.value); setImagePreview(e.target.value); }}
            ...
            <FormField control={form.control} name="category_id" render={({ field }) => (
              <FormItem>
                ...
                <Select onValueChange={(val) => { field.onChange(val); handleCategoryChange(val); }} value={field.value || ""}>
                  <FormControl>
                      ...
                      <label className="flex items-center gap-1.5 cursor-pointer">
                        <Checkbox
                          ...
                          onCheckedChange={(checked) => {
                            ...
                            field.onChange(checked ? [...current, size] : current.filter((s: string) => s !== size));
                          }}
                        />
